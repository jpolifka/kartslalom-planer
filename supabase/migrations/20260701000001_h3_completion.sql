-- H3 completion: get_my_formation_permission + duplicate_custom_formation

-- get_my_formation_permission: Gibt 'owner' | 'edit' | 'view' | null zurück.
-- Null = kein Zugriff oder Formation nicht gefunden.
...
begin
  ...
  return v_permission; -- null wenn kein Zugriff
end;
$$;

grant execute on function public.get_my_formation_permission(uuid) to authenticated;

-- duplicate_custom_formation: Erstellt eine eigene Kopie einer zugänglichen Formation.
-- Erlaubt für Owner und alle Shares (view + edit).
