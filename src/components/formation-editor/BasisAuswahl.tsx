...
const s: Record<string, React.CSSProperties> = {
  ...
  dialog: { background: "white", borderRadius: 12, padding: 28, width: 480, maxHeight: "80vh", display: "flex", flexDirection: "column", gap: 16 },
  title: { fontSize: 18, fontWeight: 700, color: "#111827" },
  subtitle: { fontSize: 14, color: "#6b7280" },
  optionRow: { display: "flex", gap: 12 },
  optBtn: { flex: 1, padding: "14px 12px", border: "2px solid #e5e7eb", borderRadius: 10, cursor: "pointer", background: "white", textAlign: "left" as const, fontSize: 14 },
  optBtnActive: { flex: 1, padding: "14px 12px", border: "2px solid #2563eb", borderRadius: 10, cursor: "pointer", background: "#eff6ff", textAlign: "left" as const, fontSize: 14 },
...
export default function BasisAuswahl({ onConfirm }: Props) {
  const [mode, setMode] = useState<"empty" | "standard">("empty");
  ...
  const canConfirm = mode === "empty" || (mode === "standard" && selectedKey !== null);
      ...
      <div style={s.dialog}>
        ...
        <div style={s.optionRow}>
          <button style={mode === "empty" ? s.optBtnActive : s.optBtn} onClick={() => setMode("empty")}>
            ...
            <div style={{ fontSize: 12, color: "#6b7280" }}>Leere Arbeitsfläche</div>
          </button>
          <button style={mode === "standard" ? s.optBtnActive : s.optBtn} onClick={() => setMode("standard")}>
            ...
            <div style={{ fontSize: 12, color: "#6b7280" }}>Existierende als Basis</div>
          </button>
        </div>
          ...
          <div>
            ...
            <div style={s.list}>
              {SELECTABLE.map((f) => (
                <div
                  key={f.key}
                  style={selectedKey === f.key ? s.listItemSel : s.listItem}
                  onClick={() => setSelectedKey(f.key)}
                >
                  {f.label}
                  <span style={{ marginLeft: 6, fontSize: 11, color: "#9ca3af" }}>
                    ({f.cones.length} Cones)
                  </span>
                </div>
              ))}
            </div>
        ...
        <button
          style={canConfirm ? s.confirmBtn : s.confirmBtnDisabled}
          disabled={!canConfirm}
